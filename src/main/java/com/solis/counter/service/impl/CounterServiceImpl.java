package com.solis.counter.service.impl;

import com.solis.counter.event.CounterEvent;
import com.solis.counter.event.CounterEventProducer;
import com.solis.counter.schema.BitmapShard;
import com.solis.counter.schema.CounterKeys;
import com.solis.counter.schema.CounterSchema;
...
public class CounterServiceImpl implements CounterService {
     ...
     * 从缓存中读取实体的各类计数指标，缓存缺失 / 损坏时安全重建计数，
     * 同时做了限流、防并发风暴、分布式锁等高可用保护
     */
    @Override
     ...
     * RBucket<Long> bucket就是提供的字符串包装类
     */
    ...
    private boolean allowedByRateLimiter(String entityType, String entityId) {
        ...
        //Redis 提供的限流工具，专门用来控制访问速率
        RRateLimiter limiter = redisson.getRateLimiter(rlKey);

        // 初始化速率（如已存在则忽略） 全局总限流，几秒内允许几次重建
     ...
     * TODO 说明：当前使用 KEYS 枚举分片（生产建议维护索引集合），结果按分片 BITCOUNT 求和。
     */
