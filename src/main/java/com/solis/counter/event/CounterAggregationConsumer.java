...
/**
 * 聚合消费者 = 专门负责 “攒数据” 和 “合并刷写” 的服务
 * 计数事件聚合与刷写消费者
 * 计数事件聚合 = 把大量高频的 +1/-1 先攒起来，不直接去改最终计数，等一会儿再统一合并更新
 * <p>职责：</p>
...
public class CounterAggregationConsumer {
    ...
    //DefaultRedisScript:把写的lua脚本包装成Redis可以执行的对象
     ...
     * 将聚合增量刷写到 SDS 固定结构计数。
     * 固定延迟 1s，保证秒级最终一致性。
     *  @ Scheduled(fixedDelay = 1000L):每隔1s执行一次
     */
    @Scheduled(fixedDelay = 1000L)
    public void flush() {
        // TODO简化实现：扫描所有聚合桶键（生产建议使用索引集合替代 KEYS）
