package com.solis.relation.service.impl;
...
import com.solis.relation.service.RelationService;
import com.solis.user.domain.User;
import com.solis.user.mapper.UserMapper;
...
public class RelationServiceImpl implements RelationService {
    ...
    public boolean isFollowing(long fromUserId, long toUserId) {
        //TODO 性能优化，先查已有的缓存，能接收短期缓存不一致风险
        String key = "uf:flws:" + fromUserId;

        // 先查 ZSet 缓存
        Double score = redis.opsForZSet().score(key, String.valueOf(toUserId));
        if (score != null) {
            return true;  // 在 ZSet 中，说明已关注
        }
    ...
    /**控制用户1s内只能发起100次关注操作，超过就拒绝
     * 初始桶中有100个令牌，以后每秒生成1个，如果一瞬间有100个请求，消耗完所有令牌，就会拒绝后续的请求
     * 同时如果桶满了，后续生成的令牌会直接丢弃
     * now：从1970到现在的秒数
     * last：上一次更新令牌时间
     * tokens,桶中令牌数
     * if not last ---第一次访问
     * 计算新增令牌数量
     * 得到新tokens
     * 用户很久不操作了，这个限流 key 就自动删掉，不占 Redis 内存
     *
    */
