package com.solis.search.service.impl;
...
import com.solis.search.api.dto.SearchResponse;
import com.solis.search.api.dto.SuggestResponse;
import com.solis.search.service.SearchService;
...
public class SearchServiceImpl implements SearchService {
    ...
    public SearchResponse search(String q, int size, String tagsCsv, String after, Long currentUserIdNullable) {
        ...
        //把前端传的After分页标记转换成ES识别的游标值，FieldValue里面能有任何类型
        List<FieldValue> afterValues = parseAfter(after);

        // 复合排序：优先相关性，其次发布时间与互动数据，最后按 content_id 稳定排序，一个SortOptions放一个排序规则
        ...
        // 完整包名，不然和自定义的 SearchResponse 冲突，ES返回类型
        //var:让编译器自动推断是什么类型
                ...
                var b = s.index(INDEX)
                        ...
                        */
                        .query(qb -> qb.functionScore(fs -> fs
                                ...
                                // 对点赞数收藏数设置权重
                                //用 (\ln(1+x)) 替代原始值，防止大V的帖子分数爆
                                //    .modifier(FieldValueFactorModifier.Log1p))平滑函数避免分差太大
     ...
     * java16+新特性：模式匹配instanceof语法
     */
    private List<String> asStringList(Object o) {
        if (o == null) {
            return Collections.emptyList();
        }
        //if(o instanceof List<?> l){}:判断是不是List类型，如果是就复制给l
